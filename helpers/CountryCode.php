...
final class CountryCode
{
     ...
     * @return array
     */
    private static function getCountryCodeList($index)
    {
        $list = [];
        foreach (static::$countryCodes as $country) {
            $list[] = $country[$index];
        }
        return $list;
    }
     ...
     * @return string
     */
    private static function getAlpha2CountryCode(string $code): string
    {
        $result = null;

        foreach (static::$countryCodes as $country) {
            if(in_array($code, $country)) {
                $result = $country[0];
                break;
            }
        }
     ...
     * @return string
     */
    public static function getAlpha2Code(string $input): string
    {
        $result = null;
        $code = mb_strtoupper($input);

        if (strlen($input) == 2) {
            $alpha2 = static::getCountryCodeList(0);

            if (in_array($code, $alpha2, true)) {
                $result = $code;
            }
        } else {
            $result = static::getAlpha2CountryCode($code);
        }
